...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        this.capture = function(response) {
            ...
            if (this.capture_options.key != undefined) {
            ...
            } else if (Array.isArray(this.sent[this.sent.length - 1].text)) {
    ...
    botkit.endConversationsWithUser = function(user_id) {
      botkit.debug('SEARCHING FOR CONVOS WITH ', user_id)

      for (var t = 0; t < botkit.tasks.length; t++) {
          for (var c = 0; c < botkit.tasks[t].convos.length; c++) {
              if (
                  botkit.tasks[t].convos[c].isActive() &&
                  botkit.tasks[t].convos[c].source_message.user == user_id
              ) {
                  botkit.debug('FOUND EXISTING CONVO - STOPPING IT, source: ', botkit.tasks[t].convos[c].source_message.text);
                  botkit.tasks[t].convos[c].stop('Interrupted')
              }
              else if (botkit.tasks[t].convos[c].source_message.user == user_id) {
                botkit.debug('FOUND INACTIVE CONVO, NOT STOPPING IT, source: ', botkit.tasks[t].convos[c].source_message.text);
              }
          }
      }
    };
